...
def collect_context_multistage(cluster_info, cfg):
    ...
    print(f"  🚨 [품질 게이트] 검증 가능한 소스 0개 — 이 글은 추측 기반이 될 위험이 높음")
    print(f"     발행 전 사람 확인 권장. 반복되면 이 주제는 기획 단계에서 제외 검토.")
...
[4단계] 단어 수 체크
  - 초안 단어 수를 카운트하세요.
  - 700단어 미만: 에러 — 각 스포크 섹션에 2~3문장 설명을 추가해서 700단어 이상으로 보강
  - 700~1000단어: 정상
  - 1000단어 초과: 경고 — 스포크 역할 침범 가능성, 어느 섹션을 축약할지 제시
    (HUB는 내비게이터 역할 — 각 스포크를 소개하되 깊게 다루지 않음)

  ⚠️ 보강 방법 (700단어 미달 시):
  - 각 스포크 섹션에 "이 글에서 다루는 것" 2~3문장 추가
    예: "This guide covers step-by-step installation, common error codes,
        and how to restore factory settings — everything you need before
        contacting support."
  - 도입부에 이 클러스터 주제가 왜 중요한지 1~2문장 추가
  - 결론에 독자 동선(어떤 글부터 읽을지) 구체적으로 명시
...
판정 리포트는 한국어로, 최종본은 영문 markdown 코드블록으로 출력하세요."""
    ...
    # ── 기존 스포크 리뷰 프롬프트 (Gemini 그라운딩용 v2) ────────────
    # 설계 원칙:
    #   - Google Search 그라운딩이 검증을 담당하므로 지시문 최소화
    #   - 판정 리포트는 문제 항목만 출력 (출력 토큰 절감)
    #   - content_type별 체크는 핵심 1줄로 압축
    type_check = {
        "GUIDE":      "- 단계별 절차·메뉴 경로가 공식 문서의 현재 버전과 일치하는지 검색으로 확인",
        "LISTICLE":   "- 제목이 약속한 목록이 실제로 3개 이상 있고, 각 항목이 현재 유효한지 검색으로 확인",
        "COMPARISON": "- 비교 양쪽이 균형있게 다뤄지고 수치·스펙이 정확한지 검색으로 확인",
        "EXPLAINER":  "- 핵심 개념 정의가 공식 소스와 일치하고 현재도 유효한지 검색으로 확인",
    }.get(content_type, "- 핵심 주장이 현재도 유효한지 검색으로 확인")

    prompt = f"""당신은 테크 블로그의 팩트체크 편집자입니다. Google 검색으로 아래 영문 초안을 검증하고 수정된 최종본을 출력하세요.

[검증 규칙]
1. 검색으로 확인할 것:
   - 구체적 수치/스펙/가격/날짜/제품명
   - [NEEDS VERIFICATION] 표시 부분 → 공식 소스로 채우거나, 확인 불가 시 문장 삭제/일반화
   - 초안의 모든 외부 URL 실존 여부 → 가짜 URL은 제거하거나 실제 URL로 교체
   - "공식 발표", "출시됨" 등의 주장 → 실제 발표 여부. 미발표면 추측/루머 프레임으로 전면 수정
{type_check}
...
3. 최종본 요구사항:
   ...
   - 분석적 편집부 톤(we/third-person), H1은 초안 제목 유지 (단, 제목이 허위 전제를 담고 있으면 수정)

[출력 형식 — 이 순서와 형식을 정확히 지키세요]
① 판정 요약 (한국어, 간결하게): 문제가 발견된 항목만 "[N] 주장 → 판정(❌/⚠️) → 수정 내용 + 근거 URL" 형식으로. 문제없는 항목은 나열하지 마세요. 전부 정상이면 "모든 검증 통과" 한 줄만.
② 최종본: ```markdown 코드블록``` 안에 완성된 영문 글 전체.

[참고]
클러스터: {cluster_name} | 허브 키워드: {hub_keyword} | 유형: {content_type}
리서치 수집 URL: {url_list}
...
def get_next_candidate_fifo():
    ...
    루프 방지: final/{slug}_{TYPE}.md가 이미 존재하면 스킵.
    (Step 3에서 생성 완료된 글은 Step 4 발행 전까지 status가
     candidate로 유지되므로, 파일 존재 여부로 완료를 판정한다.
     done은 Step 4가 실제 발행 시점에 실제 URL로 호출한다.)
    """
    data = load_pipeline()
    # 주차 오름차순 (오래된 것 먼저)
...
def record_publish(cluster_name, title, url, content_type=None, archive=True):
    """
    발행 완료 기록 — 비대화형 코어.
    Streamlit/GitHub Actions에서 직접 호출 가능.
    반환: dict (ok, error/통계)

    archive: final/ → published/ 아카이브 수행 여부.
             publish_one.py처럼 호출자가 이미 아카이브를 만들었다면
             False로 넘겨야 한다. (True로 두면 final/ 파일이 이미
             삭제된 상태에서 fallback 경로를 타 엉뚱한 파일을
             아카이브하는 버그가 발생)
    """
    data         = load_pipeline()
    ...
    archived_path = None
    if not archive:
        # 호출자가 이미 아카이브를 생성함 (publish_one.py) — 스킵
        _skip_archive = True
    else:
        _skip_archive = False
    ...
    if (not _skip_archive) and os.path.exists(final_path):
        ...
        if not os.path.exists(archived_path):
            ...
            # YAML 작은따옴표 문자열 사용 — 백슬래시(C:\Users\...)나
            # 정규식이 본문에 와도 이스케이프 문자로 해석되지 않음.
            # 작은따옴표 자체만 ''로 이스케이프하면 됨.
            _yaml_title   = title.replace("'", "''")
            _yaml_excerpt = _excerpt.replace("'", "''")
...
def cmd_done(cluster_name, title=None, url=None, content_type=None, archive=True):
    """
    CLI 래퍼 — 인자 없으면 대화형 입력 (기존 호환).
    Actions에서는 --title/--url/--type을 모두 넘겨 비대화형으로 실행.
    --no-archive: 호출자가 이미 published/ 아카이브를 만든 경우 사용.
    """
    print(f"\n{'='*60}")
    ...
    result = record_publish(cluster_name, title, url,
                            content_type=content_type, archive=archive)
...
예시:
  ...
  python write.py done "Steam Deck Windows Compatibility"
    ...
    parser.add_argument("--no-archive", dest="no_archive", action="store_true",
                        help="published/ 아카이브 생성 스킵 (done용, "
                             "publish_one.py가 이미 만든 경우)")
