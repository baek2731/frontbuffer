...
- 예시 (좋음): *"The Steam Deck's game library runs into the tens of thousands, but not all of them behave the same way on the hardware."*
- 예시 (나쁨): *"The Steam Deck has emerged as a significant player in the portable gaming landscape."*

### ② 사실 기반, 구체적 수치
- 가능하면 모델명, 버전, 날짜, 수치(온도, 용량, 해상도 등)를 문장 안에 자연스럽게 녹임
- 수치는 출처와 함께 인라인 링크로 처리 (`[출처명](URL)` 형식)
- 추상적 표현보다 구체적 사례 우선

### ③ 외부 링크: 각주 아닌 인라인
- 출처를 별도 footnote로 처리하지 않고 관련 단어/구에 앵커 텍스트로 삽입
- 예시: `as detailed in [gamermarkt's specification breakdown](URL)`
- 본문 Sources 섹션은 별도로 유지하되, 중요 근거는 본문 안에서도 링크

### ④ 결론: 실용적 takeaway
- 요약 반복 금지. 독자가 다음에 무엇을 해야 하는지 명확히 제시
- 가능하면 체크리스트나 우선순위 형태로 마무리
- 예시: *"Check the official rating first, then cross-reference with ProtonDB for titles older than six months."*

### ⑤ 콘텐츠 타입별 특성
| 타입 | 목적 | 특징 |
|------|------|------|
| GUIDE | 단계별 해결 | 번호 리스트, 구체적 절차, 경고/팁 포함 |
| EXPLAINER | 개념 이해 | 섹션별 소제목, 비교/한계점 명시, 실용 맥락 연결 |
| COMPARISON | 선택 보조 | 표 또는 항목별 병렬 구조, 결론에 추천 조건 명시 |
| LISTICLE | 목록형 정보 | 각 항목 독립적으로 읽힘, 항목당 2~4줄 |
| HUB | 클러스터 허브 | 스포크 글 전체 링크, 주제 전체 개요, 내부링크 중심 |
...
furthermore, / moreover, (남발 시) / in summary, (반복 요약)
```
...
### 🟢 완료
- publish_one.py HUB ready order 버그 수정 (hub_num → hub_num-10000)
- 톤앤매너 가이드 문서화 (v21)
- folder 자동 매핑
- file_id 전달 버그
